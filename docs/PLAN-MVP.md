...
| 9 | Close & outreach — *done* | Opening close, template drafts with merge fields, editing, sending via Resend after approval | Drafting requires the opening closed; nothing sends without a recorded name; unconfigured sending answers 503 rather than dropping the message |
| 10 | Compliance & data lifecycle — *done* | `AuditLog` endpoint, CSV export with formula neutralisation, retention on the worker tick, access and erasure endpoints | Retention deletes files and rows past the window; erasure removes résumé, evaluation and PII while the audit survives anonymised |
